...
public class GameDataRefreshService : BackgroundService
{
    private readonly IServiceProvider _serviceProvider;
    private readonly ILogger<GameDataRefreshService> _logger;
    private readonly TimeSpan _checkInterval = TimeSpan.FromMinutes(1);

    public GameDataRefreshService(
        IServiceProvider serviceProvider, 
        ILogger<GameDataRefreshService> logger)
    {
    ...
    public override async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        await ((GameDataService)gameDataService).InitializeAsync();
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                if (timeUntilMidnight.TotalMinutes < 1)
                {
                    ...
                    {
                        _logger.LogError("Scheduled midnight refresh failed after all attempts");
                    }
